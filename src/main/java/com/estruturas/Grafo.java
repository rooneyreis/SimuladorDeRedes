...
public class Grafo {
    ...
    // ====================================================================
    //  NOVO (topologia dinamica): remocao de vertices e arestas.
    //  Necessario porque agora o utilizador cria e apaga dispositivos e
    //  ligacoes em tempo real, atraves da interface web.
    // ====================================================================

    // Remove uma aresta nao-dirigida entre dois dispositivos (ambos os sentidos).
    public void removerAresta(Dispositivo origem, Dispositivo destino) {
        Vertice vOrigem = procuraVertice(origem);
        Vertice vDestino = procuraVertice(destino);
        if (vOrigem == null || vDestino == null) {
            return;
        }
        desligar(vOrigem, vDestino);
        desligar(vDestino, vOrigem);
    }

    // Remove a aresta dirigida origem->destino, se existir.
    private void desligar(Vertice origem, Vertice destino) {
        ListaDuplamenteLigada adj = origem.getAdjacentes();
        No atual = adj.getPrimeiro();
        int posicao = 0;
        while (atual != null) {
            Aresta a = (Aresta) atual.getElemento();
            if (a.getDestino() == destino) {
                adj.removePosicao(posicao);
                return;
            }
            atual = atual.getProximo();
            posicao++;
    ...
    // Remove um vertice e todas as arestas que lhe chegam ou dele partem.
    public void removerVertice(Dispositivo dispositivo) {
        Vertice alvo = procuraVertice(dispositivo);
        if (alvo == null) {
            return;
        }

        // 1. apagar todas as arestas dos OUTROS vertices que apontam para o alvo
        No atual = this.vertices.getPrimeiro();
        while (atual != null) {
            Vertice v = (Vertice) atual.getElemento();
            if (v != alvo) {
        ...
        // 2. remover o proprio vertice da lista de vertices
        int posicao = 0;
        No no = this.vertices.getPrimeiro();
        while (no != null) {
            if (no.getElemento() == alvo) {
                this.vertices.removePosicao(posicao);
                return;
            }
            no = no.getProximo();
            posicao++;
